...
public class M1Application {
	...
	class SimpleConfiguration {
		...
		@Bean
		@LoadBalanced
		AsyncRestTemplate traceAsyncRestTemplate(Tracer tracer,
				HttpSpanInjector httpSpanInjector,
				HttpTraceKeysInjector httpTraceKeysInjector, ErrorParser errorParser) {
			TraceAsyncClientHttpRequestFactoryWrapper traceAsyncClientHttpRequestFactoryWrapper = new TraceAsyncClientHttpRequestFactoryWrapper(
					tracer, httpSpanInjector,
					new AsyncRestTemplate().getAsyncRequestFactory(),
					httpTraceKeysInjector);

			return new TraceAsyncRestTemplate(traceAsyncClientHttpRequestFactoryWrapper,
					tracer, errorParser);
		}
	...
	class SimpleController {
		...
		DeferredResult<String> helloAsync() {
			...
			asyncRestTemplate.getForEntity("http://m2/hello", String.class).addCallback(
		...
		DeferredResult<String> helloAsyncRx() {
			...
			ListenableFutureAdapter
					...
					.observeOn(scheduler).map(HttpEntity::getBody)
